...
public class SceneNodeControl : MonoBehaviour {
    ...
    public AxisFrameControl TheAxisFrame = null;
    public SceneNode LastNodeWithAxisFrame = null;
    ...
    void Start() {
        ...
        Debug.Assert(TheAxisFrame != null);
        ...
        SetAxisFrameToSceneNode(mSelectedTransform[0]);
    }
    ...
    void SelectionChange(int index)
    {
        ...
        // add axis control as primitive to the active scene node
        SetAxisFrameToSceneNode(mSelectedTransform[index]);
    }

    // input parameter is a scene node's transform
    public void SetAxisFrameToSceneNode(Transform selectedTransform)
    {
        // notify the old node of deselect
        if (this.LastNodeWithAxisFrame != null)
        {
            this.LastNodeWithAxisFrame.AxisFrame = null;
        }
        // select new and set it
        this.LastNodeWithAxisFrame = selectedTransform.gameObject.GetComponent<SceneNode>();
        Debug.Assert(this.LastNodeWithAxisFrame != null);
        this.LastNodeWithAxisFrame.AxisFrame = GameObject.Find("AxisFrame").GetComponent<AxisFrameControl>();
        Debug.Assert(this.LastNodeWithAxisFrame.AxisFrame != null);
    }
}
